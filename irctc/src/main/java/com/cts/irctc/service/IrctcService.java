...
public class IrctcService {
	...
	public TrainInfo getTicketBookingResult(TicketBooking ticketBooking) throws ApplicationException {
		//Add code here..
		List<TrainInfo> trainInfo = buildTrains();
		if(ticketBooking.getFromCity().equalsIgnoreCase(ticketBooking.getToCity())) {
			...
			String tc = travelClass.substring(travelClass.length()-3, travelClass.length()-1).trim();
			for(TrainInfo ti:trainInfo) {
				if(ti.getTavelClass().equalsIgnoreCase(tc) && ti.getFrom().equalsIgnoreCase(ticketBooking.getFromCity()) && ti.getTo().equalsIgnoreCase(ticketBooking.getToCity())) {
					return ti;
				}
			}
		}
	...
	public String getPNRNumber(TicketBooking ticketBooking) throws ApplicationException {
		//Add code here..
		String secondend = ticketBooking.getMobileNumber().substring(0, 7);
		if(ticketBooking.getFromCity().equalsIgnoreCase(ticketBooking.getToCity())) {
			throw new ApplicationException();
		}
		return "-" + secondend;  //TODO, modify return value
